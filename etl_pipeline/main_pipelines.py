    ...
    try:
        ...
        con.execute(f"""
            ...
            SELECT
                ...
                -- -----------------------------------------------------------
                CASE 
                    WHEN TRY_CAST(ArrDelay AS INTEGER) > 0 THEN TRY_CAST(ArrDelay AS INTEGER) * 75.0 
                    ELSE 0.0 
        ...
        con.execute(f"""
            COPY (
                SELECT 
                    Flight_ID, FlightDate, Tail_Number, Origin, Dest, 
                    Reporting_Airline, AirTime, Cancelled, Distance, 
                    DepDelay, Diverted, MRL_Liability, Estimated_Cost, Flights, ArrDelay
                FROM bts_flights
            ) 
        ...
        """)
        # ── Export Fact Delay Table (Properly Unpivoted) ───────────────────────
        delay_path = os.path.join(DATA_PROCESSED, 'Aviation_Fact_Delay_Table.parquet')
        print("  → Exporting Unpivoted Delay Fact Table...")
        con.execute(f"""
            COPY (
...
if __name__ == "__main__":
    ...
    print("    └── Aviation_Fact_Delay_Table.parquet")
    print("="*64 + "\n")
